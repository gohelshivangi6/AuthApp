...
import {
  fetchUsers, fetchDepartments, fetchWidgets, fetchDashboards,
  fetchPermissions, createPermission, deletePermission,
} from "../../redux/slices/adminSlice";

export default function PermissionManager() {
  const dispatch = useDispatch();
  const { users, departments, widgets, dashboards, permissions, loading } = useSelector((state) => state.admin);
  ...
  return (
    <Box>
      <Typography variant="h6" sx={{ fontFamily: "Outfit", fontWeight: 700 }} mb={2}>
        Department, Widget & Dashboard Permissions
      </Typography>
          ...
          <Paper sx={{ p: 2, background: "rgba(255,255,255,0.02)" }}>
            ...
            {dashboards.map((d) => (
              <FormControlLabel
                ...
                control={
                  <Switch
                    checked={isGranted("dashboard", d.id)}
                    onChange={() => handleToggle("dashboard", d.id, isGranted("dashboard", d.id))}
                  />
                }
                label={
                  <Box display="flex" alignItems="center" gap={1}>
                    {d.name}
                    <Chip
                      label={isGranted("dashboard", d.id) ? "Granted" : "Denied"}
                      size="small"
                      color={isGranted("dashboard", d.id) ? "success" : "error"}
                      variant="outlined"
                    />
                  </Box>
                }
